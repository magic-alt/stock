...
### Added
- V6 Phase 5 (Plugin SPI + SDK): add `src/sdk/` as the plugin-author facade, a V6 `PluginRegistry` with manifest loading, entry-point metadata, permission gating, and conformance testing, plus the `quant-platform` console script with `plugin test`.
- Plugin authoring: add a cookiecutter plugin template under `templates/cookiecutter-plugin/` and a runnable `examples/plugins/simple_momentum_plugin/` strategy plugin.
- Architecture (V6 Phase 4 — Adapters convergence): add canonical adapter namespaces under `src/adapters/{data,realtime,broker,storage,ml,messaging}/` with compatibility re-exports for existing data source, realtime, broker gateway, storage, MLOps, and message bus implementations.
...
### Tests
- Add V6 Phase 5 SDK / PluginRegistry / CLI tests covering manifest validation, permission gating, example plugin conformance, cookiecutter template shape, and the `quant-platform` console script entry.
