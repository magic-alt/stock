...
def test_api_v2_accounts_transfer_and_allocation_preview():
...
def test_api_v2_allocation_preview_requires_accounts():
    app = create_app(enable_cors=False)

    with TestClient(app) as client:
        response = client.post(
            "/api/v2/portfolio/capital-allocation/preview",
            json={"tenant_id": "missing", "strategy_weights": {"s1": 1.0}},
        )
