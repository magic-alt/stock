...
| 策略库 (26个) | ✅ 丰富 | 经典技术/ML/专用策略齐全，缺少策略调参文档 |
| 数据源 (4个 provider) | ⚠️ 中高 | QLib provider 为 stub（回退到 YFinance）、无实时流数据 |
| 仿真引擎 | ✅ 高成熟度 | 完整订单生命周期，已实现 A 股规则（T+1/涨跌停/停牌/整手），部分成交概率模型 |
| 因子管道 (2文件) | ⚠️ 中等 | 15+ 因子，但缺少基本面数据因子和跨因子相关性分析 |
| Web 前端 | ⚠️ 可用 | 已实现状态管理、表单验证、通知、自动刷新、WebSocket 推送，缺少图表可视化 |
| 编排器 | ⚠️ 可用 | 顺序工作流可运行，缺少持久化集成和调度能力 |

### 测试覆盖

| 维度 | 状态 |
|------|------|
| 测试文件 | 39 个，覆盖核心/平台/ML/运维 |
| 缺失 | 无负载测试、无故障场景测试、无端到端集成测试、无性能回归测试 |
