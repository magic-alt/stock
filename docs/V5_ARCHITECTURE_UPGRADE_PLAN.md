...
| 授权控制 | 9权限 + account/strategy隔离 | 8/10 | 缺少细粒度资源级ACL、动态权限策略 |
...
**结论**: DuckDB 最适合本系统定位（本地部署、单机为主、嵌入式优先）
